...
export default function SeriesDetailScreen() {
  ...
  async function fetchDetail() {
    try {
      const { data: json } = await api.get<MediaDetail>(`/media/tv/${id}`, {
  ...
  async function handleAddToCollection() {
    ...
    try {
      await api.post('/movies/store-from-api', {
        tmdb_id: id, title: data.title, poster: data.poster, media_type: 'tv',
      }, { headers: await authHeaders() });
      Alert.alert('Sucesso', 'Série adicionada à coleção!');
      fetchDetail();
    } catch (error: any) {
  ...
  async function handleSaveReview() {
    ...
    try {
      await api.post('/movies/save-or-update', {
        ...
        media_type: 'tv', user_rating: userRating, review,
      }, { headers: await authHeaders() });
